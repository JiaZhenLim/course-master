        ...
        <ul class="breadcrumb">
            ...
            <li>
                <c:if test="${sessionScope.user.identity == 0 }">
                    <a href="javascript:void(0);" onclick="selectUser(${pageNum})">用户管理
                    </a>
                </c:if>
                ...
                <div class="form-group" style="float:right;padding:0px 20px 0px 0px">
                    ...
                    <c:if test="${sessionScope.user.identity == 0 }">
                        ...
                        <select name="identity" class="js_select_role" id="form-field-1">
                            ...
                            <option value="1" <c:if test="${identity == 1}">selected</c:if>>组织或社团管理员</option>
                            <option value="0" <c:if test="${identity == 0}">selected</c:if>>系统管理员</option>
                        </select>

                        <c:if test="${identity ==2 }">
                            <select name="collegeid" value="${collegeid }" class="js_select_class" id="form-field-1">
                                <option value="-1" selected>所有</option>
                                <c:forEach items="${college }" var="colleges">
                                    <option value="${colleges.id }"
                                            <c:if test="${colleges.id == collegeid}">selected</c:if>>${colleges.name }</option>
                                </c:forEach>
                            </select>
                        </c:if>
                    </c:if>
                        ...
                        <select name="classid" value="${classid }" class="js_select_class" id="form-field-1">
                            ...
                            <c:forEach items="${classes }" var="clazz">
                                <option value="${clazz.id }"
                                        <c:if test="${clazz.id == classid}">selected</c:if>>${clazz.name }</option>
                            </c:forEach>
                        </select>
                    </c:if>--%>
                    <!-- 系统管理员显示的下拉框结束 -->
                    ...
                    <c:if test="${sessionScope.user.identity == 0}">
											<span class="input-icon">
												<input type="text" value="${search }" placeholder="请输入编号或者姓名 ..."
                                                       class="js_search nav-search-input" for="nav-search-input"
                                                       autocomplete="off"/>
                            ...
                            <table id="sample-table-1" class="table table-striped table-bordered table-hover">
                                ...
                                <tr>
                                    ...
                                    <c:if test="${identity ==2 }">
                                        ...
                                        <th class="js_th">性别</th>
                                    </c:if>

                                    <c:if test="${identity ==1 }">
                                        <th class="js_th">所属学院</th>
                                    </c:if>
                                ...
                                <c:forEach items="${users}" var="user">
                                    <tr>
                                        ...
                                        <c:if test="${identity == 1 }">
                                            <th class="js_th">${user.collname}</th>
                                        </c:if>
                                            ...
                                            <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                                ...
                                                <c:if test="${sessionScope.user.identity == 1}">
                                                    <button class="btn  btn-sm btn-danger "
                                                            onclick="deleteStudent(${user.studentCourseId},${user.courseId})">
                                                        删除学生
                                                    </button>
                                                    <button class="btn  btn-sm btn-purple "
                                                            onclick="addStudent(${user.courseId})">添加学生
                                                    </button>
                                                </c:if>
                                            </div>
                        ...
                        <ul class="pagination pull-right no-margin">
                            ...
                            <c:if test="${pageNum+4 <= totalPage}">
                                <c:forEach var="page" begin="${pageNum}" end="${pageNum +4 }">
                                    <li>
                                        <c:if test="${sessionScope.user.identity == 0 }">
                                            <a href="javascript:void(0);" onclick="selectUser(${page})"
                                               value="${page }">${page }</a>
                                        </c:if>
                                        <c:if test="${sessionScope.user.identity == 1 }">
...
<script type="text/javascript">
    ...
    function selectStudent(pageNum) {
        ...
        window.location.href = "studentListBySearch.do?pageNum=" + pageNum + "&pageSize=10&search=" + search + "&courseId=" + courseId + "&teacherId=" + teacherId;
    }
    ...
    function selectUser(pageNum) {
        ...
        if (identity == 2) {
            ...
            var collegesid = $(".js_select_class").val();
            if (collegesid != undefined && collegesid != null) {
                collegeid = collegesid;
            }
        }
        window.location.href = "userListBySearch.do?pageNum=" + pageNum + "&pageSize=10&search=" + search + "&identity=" + identity + "&collegeid=" + collegeid;
