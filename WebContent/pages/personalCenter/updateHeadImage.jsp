                ...
                <form class="form-horizontal" role="form" action="updateHeadImage.do" enctype="multipart/form-data"
                    ...
                    <div class="form-group">
                        <div style="text-align: center;margin-bottom: 10px">
                            <img src="${ctx }/${sessionScope.user.imageUrl}" class="js_imgUrl img-circle" width="140"
                                 height="140">
                        </div>
                    </div>
                    ...
                    <div style="margin-bottom: 10px">
                        <div style="text-align: center;margin-bottom: 10px">
                            <button class="btn btn-info" type="button" onclick="$('input[class=js_file]').click();">
                                <i class="icon-picture bigger-110"></i>选择要上传的文件
                            </button>

                            <input class="js_file" id="js_upload_file" style="display:none" type="file"
                                   accept="image/gif, image/jpeg, image/jpg, image/png" name="imgFile">

                            &nbsp; &nbsp; &nbsp;
                            <button class="btn btn-success" type="submit">
                                <i class="icon-arrow-up bigger-110"></i>确定更改
                            </button>
    ...
    $(function () {
        ...
        $('.js_file').on('change', function () {
            ...
            $.each(this.files, function () {
                fileNames += '<span class="am-badge">' + "选中的文件：" + this.name + '</span>';
            });
            $('#file-list').html(fileNames);

            //创建文件url，显示效果
            var url = window.URL.createObjectURL(this.files[0]);
            $('.js_imgUrl').attr("src", url);
        });
    });
