...
//	}
	...
	public Map<String, Object> selectAllUser(int pageNum, int pageSize,String whereSql){
		...
		sql.append("select  u.*,date_format(u.createTime,'%Y-%m-%d') time,c.name cname,m.name mname,coll.name collname from user u "
				+ "left join  classes c on u.classId=c.id left join  major m on c.majorid=m.id left join college coll on u.collegeid=coll.id ").append(whereSql).
		...
		countSql.append("select count(*) from  user u left join  classes c on u.classId=c.id left join  major m on c.majorid=m.id left join college coll on u.collegeid=coll.id").append(whereSql);
	...
	public Map<String, Object> selectAllStudent(int pageNum, int pageSize,String whereSql){
		...
		sql.append("select  sc.id studentCourseId,sc.courseId,sc.grade,u.*,date_format(u.createTime,'%Y-%m-%d') time,co.name courseName,"
				+ "co.id courseId, co.name coursename,c.name cname,m.name mname, coll.name collname from user u inner join studentCourse sc on u.id=sc.studentId "
				+ "inner join course co on co.id=sc.courseId "
				+ "left join  classes c on u.classId=c.id left join  major m on c.majorid=m.id left join college coll on u.collegeid=coll.id ").append(whereSql).append(" order by u.createTime desc limit " +start + ","+ pageSize);
		System.out.println(sql);
		List<Map<String, Object>> listMap = super.findManyBySql(sql.toString());
	...
	public Map<String,Object> selectUserByTeacher(int pageNum, int pageSize,String whereSql){
		int start = (pageNum -1) * pageSize;
		StringBuffer sql = new StringBuffer();
		sql.append("select  sc.id studentCourseId,sc.courseId,u.*,date_format(u.createTime,'%Y-%m-%d') time,cl.name cname,m.name mname, coll.name collname"
				+ " from  user u inner JOIN studentCourse sc on u.id=sc.studentid inner join course c on c.id=sc.courseid "
				+ "left join  classes cl on u.classId=cl.id left join  major m on cl.majorid=m.id left join college coll on u.collegeid=coll.id ").append(whereSql).append(" limit " +start + ","+ pageSize);
		System.out.println(sql);
		List<Map<String, Object>> listMap = super.findManyBySql(sql.toString());
		...
		countSql.append("select count(*) from  user u inner JOIN studentCourse sc on u.id=sc.studentid inner join course c on c.id=sc.courseid "
				+ "left join  classes cl on u.classId=cl.id left join  major m on cl.majorid=m.id left join college coll on u.collegeid=coll.id ").append(whereSql);
		System.out.println(countSql);
		long count = super.getCount(countSql.toString());
		Map<String, Object> resultMap =  new HashMap<String, Object>();
		resultMap.put("listMap", listMap);
